...
static bool pathIsFile( const char* filePath )
{
...
static void readFile( const char* filePath, uint8_t* fileBuffer, int fileSize )
{
    FILE* fp = fopen( filePath, "r" );
    fread( fileBuffer, sizeof( uint8_t ), fileSize, fp );
    fclose( fp );
}

static uint32_t crcLookup[ 256U ] =
{
...
static uint32_t calculateChecksum( uint8_t* fileBuffer, uint32_t fileSize )
{
    uint32_t i;
    uint32_t currByte;
    uint32_t sum = 0U;

    for ( i = 0U; i < fileSize; i++ )
    {
        currByte = fileBuffer[ i ];
        sum = ( sum << 8U ) ^ crcLookup[ ( sum >> 24U ) ^ currByte ];
    }

    while( fileSize != 0U )
    {
        currByte = fileSize & 0x000000FFU;
        sum = ( sum << 8U ) ^ crcLookup[ ( sum >> 24U ) ^ currByte ];
        fileSize >>= 8U;
    }
...
int run_cksum( int argc, char* argv[ ] )
{
    int result = 0;
    ...
    for( i = 1; i < argc; i++ )
    {
        ...
        else if( !pathIsFile( filePath ) )
        {
            fprintf( stderr, "cksum: %s: Is a directory\n", filePath );
            result = 1;
        }
        else
        {
